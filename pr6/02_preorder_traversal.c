...
void preorder(tree* t) {
    ...
    while(1) {
        if (temp_t->pLeft) {
            if (find(pList, temp_t->pLeft->value) == NULL) {
                temp_t = temp_t->pLeft;
                push_back(pList, temp_t);
            } else if (temp_t->pRight) {
                if (find(pList, temp_t->pRight->value) == NULL) {
                    temp_t = temp_t->pRight;
                    push_back(pList, temp_t);
                } else {
                    temp_t = temp_t->pParent;
                }
            } else if (temp_t->pParent) {
                temp_t = temp_t->pParent;
            } else {
                if (temp_t->pRight) {
                    temp_t = temp_t->pRight;
